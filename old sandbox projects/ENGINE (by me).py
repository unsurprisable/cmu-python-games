...
class Camera:
    ...
    def UpdateOffsets(self):
        for gameObject in app.gameObjectList:
            ...
            gameObject.transform.offset.x = self.offset.x
            gameObject.transform.offset.y = self.offset.y
...
### DEBUG
# Tracks a variety of statistics that can help you spot unoptimized parts of your game
# To turn the UI on, simply make an instance of this somewhere in your code by typing: "Debug()"
class Debug(GameObject):
    def __init__(self, left=5, top=5):
        super().__init__()
        self.background = Rect(5, 5, 120, 140, opacity=20)
        
        self.tps = Label(f"{app.stepsPerSecond} | TPS", 999, 15, size=14, fill='white')
        self.timer = 0
        self.sampleLength = 0.25
        self.counter = 0
        
        self.shapes = Label(f"{len(app.group)} | Shapes", 999, 40, size=14, fill='white')
        
        self.gameObjects = Label(f"{len(app.gameObjectList)} | Objects", 999, 65, size=14, fill='white')
        self.unrenderedGameObjects = Label(f"{len(app.gameObjectList)} | Unrendered", 999, 80, size=14, fill='white')
        
        self.animations = Label(f"{len(app.animationList)} | Animations", 999, 105, size=14, fill='white')
        
        self.buttons = Label(f"{len(app.buttonList)} | Buttons", 999, 130, size=14, fill='white')
        ...
        self.group = Group(
            ...
            self.shapes,
            self.gameObjects,
            self.unrenderedGameObjects,
            self.animations,
            self.buttons
            )
            
        self.group.left = left
        self.group.top = top
    ...
    def Update(self):
        ...
        self.tps.toFront()
        self.tps.left = self.background.left + 5
        self.timer += Time.deltaTime
        self.counter += 1
        if (self.timer >= self.sampleLength):
            tpsValue = Clamp(self.counter / self.sampleLength, max=app.stepsPerSecond)
            self.tps.value = f"{int(tpsValue)} | TPS"
            self.timer = 0
            self.counter = 0
            
        self.shapes.toFront()
        self.shapes.left = self.background.left + 5
        self.shapes.value = f"{len(app.group)} | Shapes"
        
        self.gameObjects.toFront()
        self.gameObjects.left = self.background.left + 5
        self.gameObjects.value = f"{len(app.gameObjectList)} | Objects"
        self.unrenderedGameObjects.toFront()
        self.unrenderedGameObjects.left = self.background.left + 5
        self.unrenderedGameObjects.value = f"{len(app.gameObjectList) - app.unrenderedGameObjects} | Rendered"
        
        self.animations.toFront()
        self.animations.left = self.background.left + 5
        self.animations.value = f"{len(app.animationList)} | Animations"
        
        self.buttons.toFront()
        self.buttons.left = self.background.left + 5
        self.buttons.value = f"{len(app.buttonList)} | Buttons"
...
### Here is an example of a basic "Open World" game! Feel free to completely remove this code.  |  ###
### Here is an example of a basic "Open World" game! Feel free to completely remove this code.  |  ###
### Here is an example of a basic "Open World" game! Feel free to completely remove this code.  V  ###

# Just creates a statistics UI to track performance
Debug()

# Creates a 'Player' class that inherits 'GameObject'
class Player(GameObject):
    # Overrides the "Awake()" function of 'GameObject'. Remember! This code runs when an instance of this class is created.
    def Awake(self):
        self.visual = Circle(0, 0, 12, fill='white', border='black') # Overrides 'GameObject.visual' to add a sprite that updates based on the object's Transform
        self.collider = Circle(0, 0, 12, opacity=0) # Overrides 'GameObject.collider'; used as a hitbox other than the visual (for collisions)
        
        self.transform.position = Vector2(0, 0) # Sets the player's 'transform.position' to the center of the screen. Notice how the visual automatically updates.
        
        self.moveSpeed = 350 # Used to easily control the speed that the player moves at. Make it higher to make the player move faster
        
    # Overrides the "Update()" function of 'GameObject'. Remember! This code runs every frame.
    def Update(self):
        # Local Vector2 variable to store the direction of input that the player is using
        inputDir = Vector2()
        
        # Utilizes the 'Input' class instead of writing this all the way down in "onKeyHold"
        if (Input.GetKey('w')):
            inputDir.y += 1
        if (Input.GetKey('s')):
            inputDir.y -= 1
        ...
        # Controls the state of animation of the player (these animations are defined below this player class)
        if (inputDir.magnitude == 0):
            # If the player is not moving, make sure the walking animation is stopped and play the idle one.
            PlayerWalkAnimation.Stop()
            PlayerIdleAnimation.Play()
        else:
            # If the player IS moving, make sure the idle animation is stopped and play the walking one.
            PlayerIdleAnimation.Stop()
            PlayerWalkAnimation.Play()
            
        # Updates the player's position vector based on: (the normalized input direction) * (the player's move speed) * (Time.deltaTime)
        # The values are multiplied by "Time.deltaTime" to keep the magnitude of movement consistent between any framerate
        # (Try it! Change the value of "app.stepsPerSecond" and see how the player moves the same amount over time!)
        moveDir = Vector2(inputDir.x, inputDir.y).normalized * (self.moveSpeed * Time.deltaTime)
        self.transform.position.x += moveDir.x
        self.transform.position.y -= moveDir.y
        
        # Lerps (linearly interpolates) the 'mainCamera.position' to make it smoothly follow the player.
        # The '7' is just to give it a speed; that value can be anything
        mainCamera.position.x = Lerp(mainCamera.position.x, self.transform.position.x, 4 * Time.deltaTime)
        mainCamera.position.y = Lerp(mainCamera.position.y, self.transform.position.y, 4 * Time.deltaTime)


# Creates a class that inherits 'Animation' to be used to animate the player when they are not moving.
class PlayerIdleAnimation(Animation):
    def Keyframes(self):
        return [
            # These keyframes all use the key 'scale.x', meaning they are grouped together under that key.
            Keyframe('scale.x', 1, 0),                          # Value: 1,     Time: 0 sec
            Keyframe('scale.x', 1.2, .5, Easing.SIN_IN_OUT),    # Value: 1.2,   Time: 0.5 sec
            Keyframe('scale.x', 1, 1, Easing.SIN_IN_OUT),       # Value: 1,     Time: 1.0 sec
            
            # These keyframes all use the key 'scale.y', meaning they are grouped together under that key.
            Keyframe('scale.y', 1.1, 0),                        # Value: 1.1,   Time: 0 sec
            Keyframe('scale.y', .9, .5, Easing.SIN_IN_OUT),     # Value: 0.9,   Time: 0.5 sec
            Keyframe('scale.y', 1.1, 1, Easing.SIN_IN_OUT),     # Value: 1.1,   Time: 1.0 sec
        ]
            
    # MUST declare this method when creating an animation
    def Keys(self):
        # When the animation cycles through the keyframes under 'scale.x', it directly modifies the value of 'player.transform.scale.x'
        player.transform.scale.x = self.SetKey('scale.x')
        # When the animation cycles through the keyframes under 'scale.y', it directly modifies the value of 'player.transform.scale.y'
        player.transform.scale.y = self.SetKey('scale.y')
        
PlayerIdleAnimation = PlayerIdleAnimation() # Must declare a single instance of the animation to use it.
PlayerIdleAnimation.looping = True # Sets it so the animation will infinitely loop until manually stopped, using '.Stop()'

# Same thing as the animation above, just using different values.
class PlayerWalkAnimation(Animation):
    def Keyframes(self):
        return [
            Keyframe('scale.x', 1.1, 0),
            Keyframe('scale.x', 1, .085),
            Keyframe('scale.x', 1.1, .17),
            
            Keyframe('scale.y', 1, 0),
            Keyframe('scale.y', 1.3, .085),
            Keyframe('scale.y', 1, .17),
            ]
...
PlayerWalkAnimation.looping = True


# Makes an instance of the 'Player' class.
player = Player()


# Simple box class (GameObject) to help visualize the dynamic camera system
class Box(GameObject):
    def __init__(self, position):
        super().__init__() # <-- Remeber to do this when working with a GameObject!
        
        self.visual = Rect(0,0, 50, 50, fill='chocolate', border='black')
        
        self.transform.position = position
    
# Instantiates multiple box visuals at different positions
box1 = Box(Vector2(125, 110))
box2 = Box(Vector2(90, -125))
box3 = Box(Vector2(-80, -75))
box4 = Box(Vector2(-125, 130))
box5 = Box(Vector2(-225, -250))
box6 = Box(Vector2(10, -300))
box7 = Box(Vector2(250, -240))
box8 = Box(Vector2(310, -25))
box9 = Box(Vector2(260, 235))
box10 = Box(Vector2(15, 350))
box11 = Box(Vector2(-275, 210))
box12 = Box(Vector2(-310, -20))


# And that's it! With only 78 lines of actual code (excluding comments & blank spaces),
# we have constructed two full animation states, have a player that can move around with
# 'WASD' while utilizing those two animations, a simulated Camera that follows the player
# around (allowing 'Open World' exploration), and some boxes set up that react to the camera.


Label ("'WASD to move'", 262.5, 18, size=30, fill='white', bold=True)
